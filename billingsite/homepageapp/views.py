...
from loginapp.models import homeMOTD, Roommates, Payments
from loginapp.forms import homeMOTDform
...
def home(request):
    ...
    webinfo = homeMOTD.objects.filter(id__gte=1)[0]
    context = {
        'sitename':"720 Bills",
...
def editwebsite(request):
    ...
    if request.method=='POST':
        form = homeMOTDform(request.POST, request.FILES, instance=webinfo)
        if form.is_valid():
            webinfo = form.save(commit=False)
            webinfo.save()
            return HttpResponseRedirect("/home/")
    else:
        form = homeMOTDform(instance=webinfo)
    context = {
        'webinfo':webinfo,
        'form':form,
    }
